...
class BrowserAgent:
    ...
    def _ensure_browser(self):
        ...
        if not self._page:
            if not HAS_PLAYWRIGHT:
                print("❌ Playwright not installed. Please run 'pip install playwright' and then 'playwright install' in your terminal.")
                return False
            try:
                print("🌐 Initializing Browser Engine (Playwright)...")
    ...
    def open_url(self, url):
        ...
        try:
            ...
            self._page.goto(url, wait_until="domcontentloaded", timeout=30000) # type: ignore
    ...
    def search_and_browse(self, query):
        ...
        print(f"🔍 Searching for: {query} using DDGS...")
        try:
            with DDGS() as ddgs:
                ...
                print(f"🎯 Top Result: {title} ({target_url})")
    ...
    def extract_page_data(self):
        ...
        try:
            print("🧪 Extracting page data intelligently...")
    ...
    def interact(self, action_type, selector, value=None):
        ...
        try:
            ...
            return f"❌ Interaction failed on `{selector}`: {str(e)}"
...
def cmd_browser_fill(args):
    ...
    if len(parts) < 2: return "I need both a selector and the text to type! ⌨️"
