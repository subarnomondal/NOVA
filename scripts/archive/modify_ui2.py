...
def inject_html():
    try:
        ...
        # Find the exact span of the avatar
        idx = content.find('<div class="w-8 h-8 rounded-full border-2 border-primary overflow-hidden')
        if idx != -1:
            group_idx = content.rfind('<div class="relative group">', 0, idx)
            if group_idx != -1:
                new_avatar_start = '''<div class="relative group" id="avatar-container">
<div id="avatar-btn" class="w-8 h-8 rounded-full border-2 border-primary overflow-hidden shadow-[0_0_10px_rgba(221,183,255,0.5)] cursor-pointer hover:scale-105 transition-transform">'''
                inner_div_end = content.find('>', idx) + 1
                content = content[:group_idx] + new_avatar_start + content[inner_div_end:]
...
<div id="avatar-dropdown" class="absolute right-0 mt-2 w-48 rounded-xl glass-panel shadow-2xl border border-white/10 hidden flex-col overflow-hidden" style="z-index: 100;">
    ...
    <div class="p-3 hover:bg-white/5 cursor-pointer text-on-surface text-sm transition-colors">⚙️ Preferences</div>
                ...
                content = content[:second_last_div] + dropdown + content[last_div + 6:]
                print('Avatar injected successfully!')
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (sidebarToolsBtn) {
            sidebarToolsBtn.parentElement.parentElement.addEventListener('click', () => {
                ...
                if (toolWeb) toolWeb.scrollIntoView({behavior: 'smooth', block: 'center'});
            });
        }
